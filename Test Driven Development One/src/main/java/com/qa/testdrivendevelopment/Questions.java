...
public class Questions {
    ...
    public String multiChar(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            result = result.concat(Character.toString(input.charAt(i)));
            result = result.concat(Character.toString(input.charAt(i)));
            result = result.concat(Character.toString(input.charAt(i)));
        }
        return result;
    ...
    public String sandwichFilling(String sandwich) {
        String lowerCaseSandwich =  sandwich.toLowerCase();
        int first = lowerCaseSandwich.indexOf("bread");
        int last = lowerCaseSandwich.lastIndexOf("bread");
        if (first == last) {
            return "";
        }
        String cutString = lowerCaseSandwich.substring(first + 5, last);
        String reverse = new StringBuffer(cutString).reverse().toString();
        return reverse;
    }
    ...
    public boolean evenlySpaced(int a, int b, int c) {
        int[]orderList = new int[] {a, b, c};
        Arrays.sort(orderList);
       
        int firstDiff = orderList[1] - orderList[0];
        int secDiff = orderList[2] - orderList[1];
    ...
    public String nMid(String input, int n) {
        ...
        int difference = (length-n)/2;
        String first = input.substring(0, difference);
        String last = input.substring(length-difference, length);
        return first + last;
    }
    ...
    public boolean endsJava(String input) {
        ...
        String end = lowerInput.substring(input.length()-4);
        if (end.equals("java")){
            return true;
        }
        return false;
	...
	public int amISearch(String sentence) {
		...
		if (lowerCase.substring(0,3).equals("am ")) {
			count++;
		}
		if (lowerCase.substring(lowerCase.length()-3,lowerCase.length()).equals(" am")) {
			count++;
		}
		
		int index = lowerCase.indexOf(" am ");
		while (index != -1) {
			count++;
			lowerCase = lowerCase.substring(index+1);
			index = lowerCase.indexOf(" am ");
		}
    	return count;
	}
	...
	public int largest(String input) {
		...
		for (int i =0; i < input.length(); i++) {
			if (String.valueOf(input.charAt(i)).equals(" ")) {
				...
				int c = Integer.parseInt(String.valueOf(input.charAt(i)));	
				currentTotal += c;
			}
		}
		if (largestTotal < currentTotal) {
	...
	public boolean compares(String word, int index, char letter) {
		...
		if (index<=lowerCase.length()) {
			char letterFromWord = lowerCase.charAt(index-1);
